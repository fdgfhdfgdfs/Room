...
import { onAuthStateChanged, User, GoogleAuthProvider, signInWithRedirect, signInWithPopup, getRedirectResult } from 'firebase/auth';
import { doc, setDoc, getDoc, updateDoc, onSnapshot, deleteDoc, collection, getDocs } from 'firebase/firestore';
...
export default function App() {
  ...
  useEffect(() => {
    ...
    // هذه الدالة تم إضافتها لاصطياد تسجيل الدخول إذا قام المتصفح بعمل Refresh
  ...
  // نظام تسجيل الدخول المزدوج (للمتصفح وللتطبيق)
  const handleSignIn = async () => {
    if (isSigningIn) return;
    setIsSigningIn(true);
    try {
      const provider = new GoogleAuthProvider();
      const customUA = "Mozilla/5.0 (Linux; Android 13; SM-S918B) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/112.0.0.0 Mobile Safari/537.36";
      
      // إذا كان المستخدم يفتح اللعبة من تطبيق الموبايل (نستخدم Redirect لمنع الشاشة البيضاء)
      if (navigator.userAgent === customUA) {
        await signInWithRedirect(auth, provider);
      } 
      // إذا كان المستخدم يفتح من متصفح عادي مثل جوجل كروم (نستخدم Popup لتسريع العملية)
      else {
...
export default function App() {
  ...
  // الجزء المعدل ليعمل على المتصفح بطريقتك القديمة، والتطبيق بالطريقة الجديدة
  const handleSignIn = async () => {
    if (isSigningIn) return;
    setIsSigningIn(true);
    try {
      ...
      if (navigator.userAgent === customUA) {
        // إذا كان يفتح من تطبيق AppCreator24 نستخدم التحويل لمنع الشاشة البيضاء
        const provider = new GoogleAuthProvider();
        await signInWithRedirect(auth, provider);
      } else {
        // إذا كان يفتح من أي متصفح عادي نستخدم الكود الأصلي الخاص بك
...
export default function App() {
  ...
  // الجزء الذكي الذي يحدد هل اللاعب في التطبيق أم متصفح عادي
  const handleSignIn = async () => {
    if (isSigningIn) return;
    setIsSigningIn(true);
    try {
      ...
      if (navigator.userAgent === customUA) {
        ...
        // إذا كان يفتح من أي متصفح عادي نستخدم النافذة المنبثقة لأنها أسرع وأفضل للينكات
        await signInWithPopup(auth, provider);
      }
    } catch (err) {
      console.error(err);
